# Executive Summary last page 259 page 219
...
'C:\\Users\\Al\\spam
```
-  `str(homeFolder / subFolder)` that way good to matches difference between OS
...
_
##### Path() return  
-  **path** return posixobject for unix os 
- Path() return  windowspath object for windows
> لازم يكون في path ()  علشان تبني عليها string path 
-
...
because changing the current working directory while a program is run-
ning can often lead to subtle bugs.
The os.getcwd() function is the older way of getting the current working
...
### 1.5 Absolute vs. Relative Paths in os module
...
> مفيد في commend injection  علشان تعرف مكان ال /etc/passwd ديه هبداية مني 
...
**Note that `mkdir()`**
- can only make one directory at a time; it won’t make several subdirectories at once like os.makedirs().
...
- **Extracting Parts**: `os.path` objects have attributes like `.dirname` (get DirName from root), `.basename` (filename )


![[Screenshot from 2026-04-14 16-21-12.png]]

![[Screenshot (88).png]]
- `os.path.split(calcFilePath)` return tuple , and `calcFilePath.split(os.sep)` return list.
```
...
- you can then start gathering information about specific files and folders
- **`os.path.getsize(path)`**: Returns the size of a file in bytes.
- **`os.listdir(path)`**: Returns a list of all files and folders in the specified directory._(Note that this function is in the os module, not os.path.)_
...
## 3.Checking Path Validtion
-    **Calling** `p.exists()` returns True if the path exists or returns False if it
doesn’t exist.

-    **Calling** `p.is_file()` returns True if the path exists and is a file, or returns
False otherwise.
...
print(dDrive.exists()) # that way to check hard exist
```
...
## 5. Saving Complex Variables 
_that need to repeat , i didn't understand_
